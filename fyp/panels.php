...
<head>
    ...
    <title>Panels Directory | EAP System</title>
    ...
    <style>
        ...
        /* Modal Split View Styles (from approvalpage.php) */
        ...
        .btn-letter { background-color: #6f42c1; color: white; border: none; }
        .btn-letter:hover { background-color: #59359a; color: white; }
    </style>
</head>
<body>
...
<div class="wrapper collapsed" id="wrapper">
  ...
  <div class="content p-4">
    <div class="page-banner d-flex justify-content-between align-items-center">
        <div>
            <h2 class="fw-bold mb-1">Panels Directory</h2>
            <p class="mb-0 opacity-75">Manage academic panel members and verified profiles.</p>
        </div>
        <div class="d-flex gap-2">
            <button onclick="window.location.reload();" class="btn btn-light rounded-pill px-4 shadow-sm">
                <i class="fas fa-sync-alt me-1"></i> Refresh
            </button>
            <a href="newpanel.php" class="btn btn-white bg-white text-primary rounded-pill px-4 shadow-sm fw-bold">
                <i class="fas fa-plus me-1"></i> New Panel
            </a>
        </div>
    </div>
        ...
        <div class="row g-3">
            <div class="col-md-4">
                <div class="input-group rounded-pill overflow-hidden border bg-light">
                    ...
                    <input type="text" id="searchInput" class="form-control border-0 bg-transparent" placeholder="Search by name..." onkeyup="applyAllFilters()">
                </div>
            </div>
            <div class="col-md-3">
                    ...
                    <?php while($row = $result->fetch_assoc()): ?>
                        <tr class="panel-row" data-level="<?= htmlspecialchars($row['level']) ?>" data-prog="<?= htmlspecialchars($row['programme']) ?>">
                            ...
                            <td class="text-center">
                                <span class="badge rounded-pill bg-light text-primary px-3"><?= htmlspecialchars($row['level']) ?></span>
                            </td>
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-2">
                                    <button class="btn btn-sm btn-outline-primary rounded-pill px-3" 
                                            onclick='showFullDetails(<?= json_encode($row, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'>Details</button>
                                    
                                    <?php if($row['status'] === 'Approved'): ?>
                                        <a href="generateletter.php?id=<?= $row['id'] ?>" target="_blank" class="btn btn-sm btn-letter rounded-pill px-3 shadow-sm">
                                            <i class="fas fa-file-pdf me-1"></i> Letter
                                        </a>
                                    <?php else: ?>
                                        <button class="btn btn-sm btn-light text-muted rounded-pill px-3 border" disabled>
                                            <i class="fas fa-lock me-1"></i> Letter
                                        </button>
                                    <?php endif; ?>
                                </div>
                            </td>
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 15px;">
            <div class="modal-header bg-dark text-white border-0">
                <h5 class="modal-title fw-bold"><i class="fas fa-user-circle me-2"></i>Panel Profile Overview</h5>
            ...
            <div class="modal-body p-0">
                <div class="review-container">
                    <div class="review-sidebar">
                        ...
                        <hr class="my-4">
                        <div id="profileBtnContainer" style="display: none;">
                            <a id="mProfileBtn" href="#" class="btn btn-primary w-100 rounded-pill py-2 shadow-sm" target="_blank">
                                <i class="fas fa-external-link-alt me-2"></i>Open Full Profile
                            </a>
                        </div>
                        <div id="pendingNotice" class="alert alert-light text-center border-0 small">
                            <i class="fas fa-lock me-1"></i> Full profile data is restricted until approval.
                        </div>
                    </div>
...
<script>
    ...
    function showFullDetails(data) {
        ...
        if (s === 'Approved') {
            ...
            document.getElementById('mProfileBtn').href = 'panelprofile.php?id=' + data.id;
        } else {
        ...
        if (data.resume_path) { 
            document.getElementById('detailsPdfFrame').src = data.resume_path + "#view=FitH"; 
        }
        bModal.show();
    }
